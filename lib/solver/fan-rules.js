...
const countWinModeFans = (acc, hand, packs, zuhelongType) => {
  ...
  // 边张、坎张、单钓将
  {
    ...
    // For zuhelong hands, calcTing doesn't work (zuhelong tiles can't form
    // regular melds). Instead, determine wait type from pack structure directly.
    // For non-zuhelong hands, use calcTing to verify single-tile wait.
    let canDetectWait;
    if (zuhelongType > 0) {
      canDetectWait = (zbm & winBitmap) === 0n;
    } else {
      const ting = calcTing(hand);
      canDetectWait = ting.length === 1;
    }
    ...
    if (canDetectWait) {
      for (let i = 0; i < packs.length; i++) {
        ...
        if (isJiang(p)) {
          acc.addFan(constants.FAN_DANDIAOJIANG, [i]);
        } else if (isShunzi(p)) {
          ...
          if (
            ...
            acc.addFan(constants.FAN_BIANZHANG, [i]);
          } else if (midRank === winRank) {
            acc.addFan(constants.FAN_KANZHANG, [i]);
          }
        }
