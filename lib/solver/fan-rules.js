...
const countWinModeFans = (acc, hand, packs, zuhelongType) => {
  ...
  // 边张、坎张、单钓将
  {
    ...
    if (zuhelongType > 0) {
      ...
      // Verify single-tile wait for zuhelong hands (matching C++
      // ting.size() == 1 check at fan.cpp:995). Build tile counts
      // excluding zuhelong tiles (one copy per zuhelong tile ID),
      // then check if any other tile can also complete the hand.
      // If so, suppress the wait fan.
      if (waitPackIdx >= 0) {
...
const markWinningTilePacks = (
  ...
  for (const packs of decompositions) {
    for (let i = 0; i < packs.length; i++) {
      ...
      // Skip fulu packs (i < handPackCount) — they were declared before
      // the win and don't contain the actual winning tile. Continue
      // searching for the decomposed pack that holds it.
      if (i >= handPackCount) {
